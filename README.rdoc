...
the current value of the computation. The parameter rest is a function that
represents the rest of the computation that takes some object as a parameter.
In our example 
  safeDot(() => object.ToString().ToString());
...
and abandon the rest of the computation. One more important point to note
is that in C#, there is actua
variable, including local variables. So,
  safeDot(() => object.ToString().ToString());
is really
  safeDot(() => SomeClassObject.object.ToString().ToString());
This suits our purposes though, as it prevents the invocation of the first
dot without getting a chance to intercept it.
...
== Other Notes
Yes, Dot.Bind is really just >>= without the type system Haskell provides. It's still surprisingly useful even if everything is casted down to object.
